...
basins = [8, 15] # 8 = Ross, 15 = Filchner-Ronne
...
with open('./latest_AIS_basins_data.pkl', 'rb') as file:
...
for basin in basins:
    ...
    for i in id:
        ...
        initialVAFpi = icesheet_d["cs568"][basin].iloc[0,55]
        
        if i == "cs568":
            
            # *(0.918/1e9) to convert from m^3 to Gt
            plt.plot(plot_data[basin].iloc[:,2] - 1850, (plot_data[basin].iloc[:,55] - initialVAFpi)*(0.918/1e9), label = runs[i], lw=0.8, color = line_cols[count], linestyle = line_stys[count])

        else:
            
            # *(0.918/1e9) to convert from m^3 to Gt
            plt.plot(plot_data[basin].iloc[:,2] - 1850, (plot_data[basin].iloc[:,55] - initialVAF)*(0.918/1e9), label = runs[i], lw=0.8, color = line_cols[count], linestyle = line_stys[count])
    ...
    if basin == 8:
        ...
        plt.title("Ross Ice Shelf Basin")
        
        plot_file_id = "Ross"
        
    elif basin == 15:

        plt.title("Filchner-Ronne Ice Shelf Basin")
    ...
    plt.savefig(f"{plot_file_id}_basin_VAFvsTime.png", dpi = 600,  bbox_inches='tight')